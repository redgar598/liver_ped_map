...
fanciest_UMAP<-function(d10x, highlight, split, label=F){
  ...
  if(split==T & !is.na(highlight)){  
    ...
    fanciest_UMAP <- fanciest_UMAP + facet_wrap(~age_condition, ncol=2)+ geom_text(aes(x = (min(plt_myeloid$UMAP_1)+(0.95*len_x_bar))-2, y = (min(plt_myeloid$UMAP_2)+(0.5*len_y_bar))-2, label=paste0("n = ",comma(CellCount))), cell_num_all, size=2)
  }else{if(split==T){
    cell_num_all<-as.data.frame(table(d10x@meta.data$age_condition))
    colnames(cell_num_all)<-c("age_condition","CellCount")
    fanciest_UMAP <- fanciest_UMAP + facet_wrap(~age_condition, ncol=2)+  geom_text(aes(x = (min(plt_myeloid$UMAP_1)+(0.95*len_x_bar))-2, y = (min(plt_myeloid$UMAP_2)+(0.5*len_y_bar))-2, label=paste0("n = ",comma(CellCount))), cell_num_all, size=2)
  }else{if(!is.na(highlight)){
    fanciest_UMAP <- fanciest_UMAP + annotate("text",x = min(plt_myeloid$UMAP_1)+(0.95*len_x_bar)-1, y = min(plt_myeloid$UMAP_2)+(0.5*len_y_bar)-2, label=paste0("n = ",comma(nrow(plt_myeloid[which(plt_myeloid$highlight==1),]))), size=2)
  }else{
    fanciest_UMAP <- fanciest_UMAP + annotate("text",x = min(plt_myeloid$UMAP_1)+(0.95*len_x_bar)-1, y = min(plt_myeloid$UMAP_2)+(0.5*len_y_bar)-2, label=paste0("n = ",comma(ncol(d10x))), size=2)
  }}}
