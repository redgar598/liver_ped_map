...
#   ggsave(plt, file=paste(here("../random_side/figures/"),name,".pdf", sep=""), w=w, h=h)
#   ggsave(plt, file=paste(here("../random_side/figures/jpeg/"),name,".jpeg", sep=""), w=w, h=h)
#   ggsave(plt, file=paste(here("../random_side/figures/png/"),name,".png", sep=""), w=w, h=h)}
# 

#' cellranger_out_loc <- here("/media/redgar/Seagate Portable Drive/random_side")
#' #cellranger_out_loc <- here("../../../projects/macparland/RE/PediatricAdult/ped_liver_map_raw")
#' 
#' 
#' meta<-read.table(here("../random_side/FRP_metadata.csv"), header=T, sep=",")
#' 
#' 
...
#'   #   meta_cell<-data.frame(cell=colnames(d10x), Sample_ID=caud)
#'   #   meta_cell_add<-merge(meta_cell, meta, by.x="Sample_ID", by.y="Sample_ID")
...
#'   #   empty_drop$Sample_ID<-d10x$Sample_ID
...
#' # nf_plts
#' # save_plts(nf_plts, "FRP_nuclear_fraction", w=6,h=4)
#' # 
...
#' 
#' 
#' save(d10x.list, file=here("/media/redgar/Seagate Portable Drive/random_side//raw_FRP_for_integration.RData"))
#' 
#' ################
...
# save_plts(pca_cellcycle, "FRP_pca_cellcycle", w=6,h=4)
# 
# pca_nfeature<-FeaturePlot(d10x, features = "nFeature_RNA",reduction = "pca", min.cutoff = "q9", pt.size=1)
# save_plts(pca_nfeature, "FRP_pca_nfeature", w=6,h=4)
            ...
            # save_plts(SCT_cluster_umap, "FRP_SCT_cluster_umap", w=6,h=4)
            # 
            # SCT_cluster_tsne<-DimPlot(d10x, reduction = "tsne", pt.size=0.25, label=T)
            # save_plts(SCT_cluster_tsne, "FRP_SCT_cluster_tsne", w=6,h=4)
            # 
            # cell_pca_SCT<-DimPlot(d10x, reduction="pca", group.by="Phase")
            # save_plts(cell_pca_SCT, "FRP_cell_PCA_afterSCT", w=6,h=4)
            # 
            # drop_pca_SCT<-DimPlot(d10x, reduction="pca", group.by="cell_status")
            # save_plts(drop_pca_SCT, "FRP_cell_PCA_afterSCT_DropletQC", w=6,h=4)
            # 
            # nFeature_UMAP_SCT<-FeaturePlot(d10x, features = "nFeature_RNA",reduction = "pca", min.cutoff = "q9", pt.size=1)
            # save_plts(nFeature_UMAP_SCT, "FRP_nfeature_UMAP_afterSCT", w=6,h=4)
            # 
            # 
            # 
            # chem_umap_sct<-DimPlot(d10x, reduction = "umap", group.by = "chemistry", pt.size=0.25)
            # save_plts(chem_umap_sct, "FRP_chem_SCT_umap", w=6,h=4)
            # 
            # age_umap_sct<-DimPlot(d10x, reduction = "umap", group.by = "AgeGroup", pt.size=0.25)+fillscale_age
            # save_plts(age_umap_sct, "FRP_age_SCT_umap", w=6,h=4)
            # 
            # Sample_ID_umap_sct<-DimPlot(d10x, reduction = "umap", group.by = "Sample_ID", pt.size=1)
            # save_plts(Sample_ID_umap_sct, "FRP_Sample_ID_SCT_UMAP", w=6,h=4)
            # 
            # 
            # ## Low quality cluster?
            ...
            # save_plts(ncount_umap_SCT, "FRP_ncount_umap_SCT", w=6,h=4)
            # 
            # nfeature_umap_SCT<-FeaturePlot(d10x, features = "nFeature_RNA", min.cutoff = "q9", pt.size=1)
            # save_plts(nfeature_umap_SCT, "FRP_nfeature_umap_SCT", w=6,h=4)
            # 
...
## Integration
###############
load("/cluster/projects/macparland/RE/random_side/raw_FRP_for_integration.RData")
...
save(d10x.combined, file="/cluster/projects/macparland/RE/random_side/FRP_integrated.RData")
...
# 
# Sample_ID_umap_sct<-DimPlot(d10x.combined, reduction = "umap", group.by = "Sample_ID", pt.size=0.25)
# save_plts(Sample_ID_umap_sct, "FRP_Sample_ID_rPCA_UMAP", w=6,h=4)
# 
# Sample_ID_split<-DimPlot(d10x.combined, reduction = "umap", group.by = "Sample_ID", split.by="Sample_ID",pt.size=0.25)
# save_plts(Sample_ID_split, "FRP_Sample_ID_facet_rPCA_UMAP", w=20,h=4)
# 
# age_split<-DimPlot(d10x.combined, reduction = "umap", group.by = "Sample_ID", split.by="AgeGroup",pt.size=0.25)
# save_plts(age_split, "FRP_age_facet_rPCA_UMAP", w=10,h=5)
# 
# 
# 
# 
...
# save_plts(key_markers, "FRP_markers_rPCA_UMAP", w=25,h=20)
# 
# #################
# ## Rough annotation
# #################
...
# save(cell_rough, file=here("data/FRP_cell_rough_maxmean.RData"))
# 
...
# ##############
# #save(d10x.combined, file=paste(here("data/"),"FRP_adult_ped_integrated.rds", sep=""))
# saveRDS(d10x.combined, file = here("../../../projects/macparland/RE/PediatricAdult/processed_data","FRP_adult_ped_integrated.rds"))
# 
# cell_label<-d10x.combined@meta.data
# #save(cell_label, file=paste(here("data/"),"FRP_adult_ped_cellRough.rds", sep=""))
# saveRDS(cell_label, file = here("../../../projects/macparland/RE/PediatricAdult/processed_data","FRP_adult_ped_cellRough.rds"))
# 
# 
# 
# 
# 
# #d10x.combined<-readRDS(here("/media/redgar/Seagate Portable Drive/ped_map_update_feb2024","FRP_adult_ped_integrated.rds"))
# d10x.combined<-readRDS(file = here("../../../projects/macparland/RE/PediatricAdult/processed_data","FRP_adult_ped_integrated.rds"))
# 
# #####
# ## plot cell types
# #####
...
# save_plts(roughcell_cluster_umap, "FRP_rPCA_roughcellType_cluster_umap", w=6,h=4)
...
#   annotate("text", x=-9, y=-14, label = paste0("n = ",comma(ncol(d10x.combined))))
# roughcell_cluster_umap
# save_plts(roughcell_cluster_umap, "FRP_rPCA_roughcellType_cluster_umap_nolab", w=6,h=4)
# 
...
# save_plts(roughcell_cluster_tsne, "FRP_rPCA_roughcellType_cluster_tsne_nolab", w=6,h=4)
# 
# ########
# ## DropletQC
# ########
...
# save_plts(nuclearfraction_cluster_umap, "FRP_rPCA_nuclearfraction_cluster_umap", w=6,h=4)
...
# save_plts(damaged_empty_cluster_umap, "FRP_rPCA_damaged_empty_cluster_umap", w=6,h=4)
# 
# #other QC
...
# save_plts(nFeature_cluster_umap, "FRP_rPCA_nFeature_cluster_umap", w=6,h=4)
# 
# 
# 
# 
...
# # ## SCINA
# # #################
# # #d10x<-readRDS(file = here("../random_side/","FRP_d10x_adult_ped_raw.rds"))
# # d10x<-readRDS(file = here("../../../projects/macparland/RE/PediatricAdult/processed_data","FRP_d10x_adult_ped_raw.rds"))
# # 
...
# # save(SCINA_cell_labels, file=here("../../../projects/macparland/RE/PediatricAdult/processed_data","FRP_adult_ped_SCINA_cell_labels.RData"))
# # 
# 
# ######################
# load(here("../../../projects/macparland/RE/PediatricAdult/processed_data","FRP_adult_ped_SCINA_cell_labels.RData"))
# #load(here("/media/redgar/Seagate Portable Drive/ped_map_update_feb2024/FRP_adult_ped_SCINA_cell_labels.RData"))
# 
...
#   annotate("text", x=-9, y=-14, label = paste0("n = ",comma(ncol(d10x.combined))))
# SCINA_cellUMAP
# save_plts(SCINA_cellUMAP, "FRP_rPCA_SCINA_umap", w=6,h=4)
# 
# 
# #############
...
# 
# pdf(file = here("figures/FRP_dot_plots.pdf"), w=10, h=10)
...
# plt_entropy_Sample_ID<-entropy_d10(d10x.combined, "Sample_ID")
# entropy_Sample_ID<-entropy_plt(plt_entropy_Sample_ID, "Sample_ID", d10x.combined)
# entropy_Sample_ID
# plt_entropy_Sample_ID<-entropy_d10(d10x.combined, "cell_status")
# entropy_Sample_ID<-entropy_plt(plt_entropy_Sample_ID, "cell_status", d10x.combined)
# entropy_Sample_ID
# 
...
# MAST_markers
# save_plts(MAST_markers, "FRP_MAST_markers_rPCA_UMAP", w=8,h=4)
# 
# 
# 
# ##########
# ## Sample_ID plots
# ##########
# d10x.combined$age_id<-paste(d10x.combined$Sample_ID, d10x.combined$AgeGroup)
...
#                                                      "C85_caud3pr Ped","C96_caud3pr Ped","C93_caud3pr Ped", "C64_caud5pr Ped",
#                                                      "C104_bx5pr Ped","C102 Ped","C105_caud5pr Ped","C113 Ped", "C115 Ped",
#                                                      "IFALD006 Ped", "IFALD073 Ped","IFALD030 Ped"))
# levels(d10x.combined$age_id)<-gsub(" ","\n", levels(d10x.combined$age_id))
# 
# Sample_ID_split<-DimPlot(d10x.combined, reduction = "umap", group.by = "CellType_rough", split.by="age_id",pt.size=0.25, ncol=5)+colscale_cellType+ggtitle("")
# save_plts(Sample_ID_split, "FRP_Sample_ID_roughCell_facet_rPCA_UMAP", w=22,h=16)
# 
...
# save_plts(age_split, "FRP_age_roughCell_facet_rPCA_UMAP", w=10,h=5)
# 
# 
# 
# ########
...
# save_plts(hep_age, "FRP_hep_markers_age_rPCA_UMAP", w=6,h=14)
# 
# 
# ##############
# ## RBC
...
# save_plts(RBC_overlap, "FRP_RBC_receptor_overlap_rPCA_UMAP", w=14,h=4)
# 
# DimPlot(d10x.combined_RBC, reduction = "umap",group.by="Sample_ID", pt.size=1)
...
# save_plts(myeloid_cluster_umap, "FRP_myeloid_cluster_umap", w=5,h=4)
# 
# myeloid_cluster_umap_Sample_ID<-DimPlot(d10x.combined_myeloid, reduction = "umap", pt.size=0.25, label=T, split.by = "age_id", ncol=5)
# myeloid_cluster_umap_Sample_ID
# save_plts(myeloid_cluster_umap_Sample_ID, "FRP_myeloid_cluster_umap_Sample_ID", w=10,h=6)
# 
# 
...
# 
# bar_Sample_ID<-ggplot(cell_cluster_count, aes(fill=seurat_clusters, y=n, x=age_id)) +
#   geom_bar(position="fill", stat="identity", color="black")+theme_bw()+th+
#   facet_wrap(~AgeGroup, scale="free_x")
# save_plts(bar_Sample_ID, "FRP_bar_Sample_ID_myeloid", w=14,h=6)
# 
...
# save_plts(myeloid_cluster_umap_nf, "FRP_myeloid_cluster_umap_nuclearfraction", w=5,h=4)
# 
...
# save_plts(myeloid_cluster_umap_cellstatus, "FRP_myeloid_cluster_umap_cellstatus", w=6,h=4)
# 
# umap_MTmyeloid<-FeaturePlot(d10x.combined_myeloid, features = "percent.mt", min.cutoff = "q9", pt.size=1)
# umap_MTmyeloid
# save_plts(umap_MTmyeloid, "FRP_umap_MTmyeloid", w=5,h=4)
# 
...
#   geom_boxplot(width=0.1, outlier.size = 0.05)+theme_bw()+th
# box_nfeature
# save_plts(box_nfeature, "FRP_box_nfeature", w=8,h=3)
# 
# 
# ############
# ## Myeloid labelling
...
# pdf(file = here("figures/FRP_dot_plots_myeloid.pdf"), w=10, h=10)
...
# save_plts(platelets_markers, "FRP_platelets_markers_cluster_umap", w=8,h=4)
# 
# 
# 
...
# save_plts(marker_mye, "FRP_myeloid_dotplot_doublets", w=20,h=5)
# DefaultAssay(d10x.combined_myeloid) <- "integrated"
# 
...
# myeloid_cluster_umap
# save_plts(myeloid_cluster_umap, "FRP_myeloid_cluster_umap_labelled", w=7,h=5)
# 
# plt_entropy_Sample_ID<-entropy_d10(d10x.combined_myeloid, "Sample_ID")
# entropy_Sample_ID<-entropy_plt(plt_entropy_Sample_ID, "Sample_ID", d10x.combined_myeloid)
# save_plts(entropy_Sample_ID, "FRP_entropy_Sample_ID_myeloid", w=15,h=10)
# 
...
# save_plts(entropy_age, "FRP_entropy_age_myeloid", w=15,h=10)
# 
# 
# 
# 
...
# bcell_cluster_umap
# save_plts(bcell_cluster_umap, "FRP_bcell_cluster_umap", w=5,h=4)
# 
...
# pdf(file = here("figures/FRP_dot_plots_bcell.pdf"), w=10, h=10)
...
# bcell_cluster_umap
# save_plts(bcell_cluster_umap, "FRP_BCell_cluster_umap_labelled", w=6,h=4)
# 
# plt_entropy_Sample_ID<-entropy_d10(d10x.combined_bcell, "Sample_ID")
# entropy_Sample_ID<-entropy_plt(plt_entropy_Sample_ID, "Sample_ID", d10x.combined_bcell)
# save_plts(entropy_Sample_ID, "FRP_entropy_Sample_ID_bcell", w=15,h=10)
# 
...
# save_plts(entropy_Treatment, "FRP_entropy_treatment_bcell", w=15,h=10)
# 
# 
# 
# ###########
...
# NK_T_umap
# save_plts(NK_T_umap, "FRP_NK_T_umap", w=8,h=6)
# 
# 
...
# save_plts(tcell_cluster_umap_cellstatus, "FRP_tcell_cluster_umap_cellstatus", w=6,h=4)
# umap_MTtcell<-FeaturePlot(d10x.combined_NK_T, features = "percent.mt", min.cutoff = "q9", pt.size=1)
# save_plts(umap_MTtcell, "FRP_umap_MTtcell", w=5,h=4)
# 
...
# pdf(file = here("figures/FRP_dot_plots_tcell.pdf"), w=10, h=10)
...
# NKT_cluster_umap
# save_plts(NKT_cluster_umap, "FRP_NKT_cluster_umap_labelled", w=6,h=4)
# 
# 
# plt_entropy_Sample_ID<-entropy_d10(d10x.combined_NK_T, "Sample_ID")
# entropy_Sample_ID<-entropy_plt(plt_entropy_Sample_ID, "Sample_ID", d10x.combined_NK_T)
# save_plts(entropy_Sample_ID, "FRP_entropy_Sample_ID_tcell", w=15,h=10)
# 
...
# save_plts(entropy_age, "FRP_entropy_age_tcell", w=15,h=10)
# 
# 
# 
# 
...
# 
# 
# #load("/media/redgar/Seagate Portable Drive/ped_map_update_feb2024/FRP_adult_ped_integrated_refinedlabels_withDropletQC.rds")
# 
# 
...
# 
# Sample_ID_split<-DimPlot(d10x.combined, reduction = "umap", group.by = "CellType_refined", split.by="age_id",pt.size=0.25, ncol=4)+colscale_cellType+ggtitle("")
# save_plts(Sample_ID_split, "FRP_Sample_ID_roughCell_facet_rPCA_UMAP_refined", w=22,h=14)
# 
# cell_num_all<-as.data.frame(table(d10x.combined@meta.data$AgeGroup))
# colnames(cell_num_all)<-c("AgeGroup","CellCount")
...
# save_plts(age_split, "FRP_age_roughCell_facet_rPCA_UMAP_refined", w=15,h=7)
# 
# 
...
# save_plts(age_condition_split, "FRP_age_condition_roughCell_facet_rPCA_UMAP_refined", w=15,h=10)
# 
# #### Fancy UMAP plots
# all_refined_cluster_umap_nolab<-DimPlot(d10x.combined, reduction = "umap", pt.size=0.25, group.by = "CellType_refined")+colscale_cellType+ggtitle("")+xlab("UMAP 1")+ylab("UMAP 2")+
#   annotate("text",x=-13, y=-13, label=paste0("n = ",comma(ncol(d10x.combined))))
...
# save_plts(fancyUMAP_all, "FRP_refined_cellType_umpa_fancy", w=7,h=5)
# 
# fancyUMAP_all_split<-fanciest_UMAP(d10x.combined,NA,T)
# save_plts(fancyUMAP_all_split, "FRP_refined_cellType_umpa_fancy_split", w=12,h=8)
# 
# 
# ## save legend fr bottom
...
#   geom_point(aes(fill=CellType_refined),size=4, shape=21)+xlab("UMAP 1")+ylab("UMAP 2")+
...
# save_plts(forlegned_plot, "Legend_fancy", w=20,h=10)
# 
# 
# #
...
# # save_plts(age_condition_split, "FRP_age_condition_roughCell_facet_rPCA_UMAP_refined_healthyonly", w=15,h=5.5)
# 
# 
# 
# 
...
# 
# entropy_Sample_ID<-entropy_plt(plt_entropy_Sample_ID, "Sample_ID", d10x.combined)
# save_plts(entropy_Sample_ID, "FRP_entropy_Sample_ID_allclusters", w=15,h=10)
# 
# entropy_age<-entropy_plt(plt_entropy_age, "AgeGroup", d10x.combined)
# save_plts(entropy_age, "FRP_entropy_age_allclusters", w=15,h=10)
# 
# entropy_chem<-entropy_plt(plt_entropy_chem, "chemistry", d10x.combined)
# save_plts(entropy_chem, "FRP_entropy_chemistry_allclusters", w=15,h=10)
# 
# entropy_Treatment<-entropy_plt(plt_entropy_treatment, "age_condition", d10x.combined)
# save_plts(entropy_Treatment, "FRP_entropy_Treatment_allclusters", w=15,h=10)
# 
# #############
...
# ##############
# save(d10x.combined, file=paste(here("../../../projects/macparland/RE/PediatricAdult/processed_data/"),"FRP_adult_ped_integrated_refinedlabels_withDropletQC.rds", sep=""))
# cell_label<-d10x.combined@meta.data
# save(cell_label, file=paste(here("../../../projects/macparland/RE/PediatricAdult/processed_data/"),"FRP_adult_ped_cellRefined_withDropletQC.rds", sep=""))
# 
# 
# 
# 
