...
source(here("../random_side/00_fanciest_UMAP.R"))
...
#load("/media/redgar/Seagate Portable Drive/random_side/FRP_integrated.RData")
load("../random_side/FRP_integrated.RData")
...
pdf(file = here("../random_side/figures/FRP_dot_plots.pdf"), w=10, h=10)
...
save_plts(Sample_ID_split, "FRP_Sample_ID_roughCell_facet_rPCA_UMAP_refined", w=14,h=14)
          ...
          # age_split<-DimPlot(d10x.combined, reduction = "umap", group.by = "CellType", split.by="AgeGroup",pt.size=0.25)+colscale_cellType+ggtitle("")+
...
GO_file = here("../random_side/Mouse_GOBP_AllPathways_with_GO_iea_January_01_2024_symbol.gmt")
...
lapply(1:length(celltypes), function(x){
  ...
  write.csv(sig_de, file=paste("../random_side/DGE/DEG_",celltypes[x],".csv", sep=""))
...
write.csv(sig_MC_celltype, file="../random_side/DGE/KC_cell_differential_expression_relaxed_fold_change.csv")
