...
DE_monte_carlo<-do.call(rbind, lapply(cell_types, function(celltype){
  print(celltype)
  load(here("data",paste(celltype,"adult_ped_diff_motecarlo_1000_covarSex.RData",sep="_")))
  DE_monte_carlo}))
...
KC_summary<-summary_tbl[grep("KC", summary_tbl$all_cells),]
ord_name_KC<-ord_name[which(ord_name%in%KC_summary$gene)]
write.csv(file=here("data","Significant_genes_adult_ped_KC.csv"),KC_summary[match(ord_name_KC,KC_summary$gene),])

RR_summary<-summary_tbl[grep("RR", summary_tbl$all_cells),]
ord_name_RR<-ord_name[which(ord_name%in%RR_summary$gene)]
write.csv(file=here("data","Significant_genes_adult_ped_RR.csv"),RR_summary[match(ord_name_RR,RR_summary$gene),])

MHC_summary<-summary_tbl[grep("MHC", summary_tbl$all_cells),]
ord_name_MHC<-ord_name[which(ord_name%in%MHC_summary$gene)]
write.csv(file=here("data","Significant_genes_adult_ped_MHC.csv"),MHC_summary[match(ord_name_MHC,MHC_summary$gene),])
...
diff_exp_all<-lapply(1:nrow(contrasts_celltype_age), function(x){
  de<-FindMarkers(d10x, ident.1 = contrasts_celltype_age[x,1], ident.2 = contrasts_celltype_age[x,2], test.use = "MAST",latent.vars=c("nFeature_RNA","Sex"), verbose=F)
...
DimPlot(d10x.combined_KCRR, label=T, group.by = "CellType_refined")+colscale_cellType+ggtitle("")+xlab("UMAP 1")+ylab("UMAP 2")+
  annotate("text",x=-11, y=-12, label=paste0("n = ",comma(ncol(d10x.combined_KCRR))))
...
FeaturePlot(d10x.combined_KCRR, features = c("CCL4","CCL3","IL1B","C1QC"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=0.1)
...
FeaturePlot(d10x.combined_KCRR, features = c("HMOX1","HSPA1A","AREG","TREM2"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=0.1)
...
FeaturePlot(d10x.combined_KCRR, features = c("HMOX1","CCL3","G0S2","FOS"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=0.1)


FeaturePlot(d10x.combined_KCRR, features = c("CCL4","AREG","IL1B","HMOX1"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=0.1)

FeaturePlot(d10x.combined_KCRR, features = c("IL1B"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=1)
FeaturePlot(d10x.combined_KCRR, features = c("AREG"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=1)
FeaturePlot(d10x.combined_KCRR, features = c("MRC1"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=1)

FeaturePlot(d10x.combined_KCRR, features = c("TREM2"),split.by = "AgeGroup", min.cutoff = "q9", pt.size=1)
...
plot_gene_UMAP<-function(gene){
  ...
  #exp_limit<-quantile(gene_exp[,1][which(gene_exp[,1]>0)], probs = 0.9)
  exp_limit<-0
  ...
  umap<-ggplot(plt_myeloid, aes(UMAP_1,UMAP_2))+
    geom_point(aes(color=log(gene_exp_limited)),size=0.5)+
    ...
    facet_wrap(~AgeGroup)+
    geom_text(aes(x = -5, y = -6, label=paste0("n = ",comma(CellCount))), cell_num_all)+
    scale_color_continuous_sequential(palette = "Viridis", rev=F, name=paste(gene, "\nExpression\n(log)"),na.value = "grey80")

  
  sig<-data.frame(CellType_refined=as.character(unique(plt_myeloid$CellType_refined)), sig="")
  sig$cell<-sig$CellType_refined
  sig$cell[which(sig$cell=="Macrophage\n(MHCII high)")]<-"Marcophage_MHCII"
  sig$sig[which(sig$cell%in%  DE_monte_carlo_sig$cell[grep(gene, DE_monte_carlo_sig$gene)])]<-"*"
  
  height_anno<-log(max(plt_myeloid$gene_exp_limited, na.rm=T)*1.2)
  
  violin<-ggplot(plt_myeloid, aes(AgeGroup,log(plt_myeloid[, which(colnames(plt_myeloid)==gene)])))+
    ...
    facet_wrap(~CellType_refined)+fillscale_age+
    geom_text(aes(x=1.5,label = sig, y=height_anno),data=sig,size=6)+
    geom_rect(xmin=1,xmax=2,ymin=height_anno,ymax=height_anno*1.005, size=0.5, alpha=0.2, fill="grey")

  plot_grid(umap,violin, rel_heights =  c(1,1), ncol=1)}


plot_gene_UMAP("IL1B")
save_plts(plot_gene_UMAP("IL1B"), "diff_exp_IL1B", w=10,h=10)

plot_gene_UMAP("AREG")
save_plts(plot_gene_UMAP("AREG"), "diff_exp_AREG", w=10,h=10)

plot_gene_UMAP("MRC1")
save_plts(plot_gene_UMAP("MRC1"), "diff_exp_MRC1", w=10,h=10)
...
plot_gene_UMAP("TREM2")
save_plts(plot_gene_UMAP("TREM2"), "diff_exp_TREM2", w=10,h=10)
...
plot_gene_UMAP("CCL4")
save_plts(plot_gene_UMAP("CCL4"), "diff_exp_CCL4", w=10,h=10)

plot_gene_UMAP("NFKBIA")
save_plts(plot_gene_UMAP("NFKBIA"), "diff_exp_NFKBIA", w=10,h=10)

plot_gene_UMAP("HLA-DRB1")
save_plts(plot_gene_UMAP("HLA-DRB1"), "diff_exp_HLA-DRB1", w=10,h=10)
