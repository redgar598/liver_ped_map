...
##############
## Differential expression with age in cluster 0
##############
...
table(d10x_raw_myeloidcluster0$age_condition)

de<-FindMarkers(d10x_raw_myeloidcluster0, ident.1 = "Adult Healthy", ident.2 = "Ped Healthy", test.use = "MAST",latent.vars=c("nFeature_RNA","Sex"), verbose=F)
sig_de<-de[which(de$p_val_adj < 0.005 & abs(de$avg_log2FC) > 1),]
sig_de[which(sig_de$avg_log2FC>0),]
sig_de[which(sig_de$avg_log2FC<0),]

d10x.combined_myeloid$cellname<-colnames(d10x.combined_myeloid)
d10x_integrated_myeloidcluster0 <- subset(d10x.combined_myeloid, subset = cellname %in% cluster0)
d10x_integrated_myeloidcluster0 <- RunPCA(d10x_integrated_myeloidcluster0, npcs = 30, verbose = FALSE)
d10x_integrated_myeloidcluster0 <- RunUMAP(d10x_integrated_myeloidcluster0, reduction = "pca", dims = 1:30)

age_markers<-FeaturePlot(d10x_integrated_myeloidcluster0, features = c("CCL3","CCL4","IL1B","HLA-DRB1"), min.cutoff = "q9", pt.size=1)
age_markers
save_plts(age_markers, "IFALD_myeloid_diff_genes_cluster0", w=7,h=6)

FeaturePlot(d10x_integrated_myeloidcluster0, features = c("CCL3","CCL4","IL1B","HLA-DRB1"), min.cutoff = "q9", pt.size=1, split.by = "age_condition")

### IFALD differential
de_IFALD<-FindMarkers(d10x_raw_myeloidcluster0, ident.1 = "Ped IFALD", ident.2 = "Ped Healthy", test.use = "MAST",latent.vars=c("nFeature_RNA","Sex"), verbose=F)
...
myeloid_GSEA
save_plts(myeloid_GSEA, "GSEA_Age_cluster0_myeloid", w=15,h=7)
...
myeloid_GSEA
save_plts(myeloid_GSEA, "GSEA_IFALD_cluster0_myeloid", w=15,h=7)


###########
## Composition myeloid "types"
###########
d10x.combined_myeloid@meta.data$CellType_refined<-as.character(d10x.combined_myeloid@meta.data$CellType_refined)
d10x.combined_myeloid@meta.data$CellType_refined[which(d10x.combined_myeloid@meta.data$seurat_clusters%in%c("4"))]<-"KC Like\n(Hepatocyte Like)"
d10x.combined_myeloid@meta.data$CellType_refined[which(d10x.combined_myeloid@meta.data$seurat_clusters%in%c("6"))]<-"KC Like (C97)"
...
save_plts(fancy_myeloid, "IFALD_myeloid_diff_cluster0_highlight", w=4,h=3)
...
cell_counts$CellType_refined<-factor(cell_counts$CellType_refined, levels=c("Macrophage\n(CLEC9A high)","Macrophage\n(MHCII high)",
                                                                            ...
                                                                            "RR Myeloid","Myeloid Erythrocytes\n(phagocytosis)","Cycling Myeloid"))
...
###############
myeloid_differential<-plot_grid(plot_gene_UMAP(d10x_integrated_myeloidcluster0,"CCL3", 0),
                     plot_gene_UMAP(d10x_integrated_myeloidcluster0,"CCL4", 0),
                     plot_gene_UMAP(d10x_integrated_myeloidcluster0,"IL1B", 0),
                     plot_gene_UMAP(d10x_integrated_myeloidcluster0,"HLA-DRB1", 0))
myeloid_differential
save_plts(myeloid_differential, "IFALD_myeloid_diff_genes_myeloidonly_tidy", w=7,h=5)


myeloid_differential_violin<-plot_grid(plot_gene_violin(d10x_integrated_myeloidcluster0,"CCL3"),
                                plot_gene_violin(d10x_integrated_myeloidcluster0,"CCL4"),
                                plot_gene_violin(d10x_integrated_myeloidcluster0,"IL1B"),
                                plot_gene_violin(d10x_integrated_myeloidcluster0,"HLA-DRB1"))
myeloid_differential_violin
save_plts(myeloid_differential_violin, "IFALD_myeloid_diff_genes_myeloidonly_violin", w=8,h=9)



myeloid_differential_violin<-plot_grid(plot_gene_violin(d10x_integrated_myeloidcluster0,"LY96"),
                                       plot_gene_violin(d10x_integrated_myeloidcluster0,"CETP"),
                                       plot_gene_violin(d10x_integrated_myeloidcluster0,"CCL4"),
                                       plot_gene_violin(d10x_integrated_myeloidcluster0,"CTSB"))
myeloid_differential_violin
...
save_plts(myeloid_age_heat, "Myeloid_age_heat", w=10,h=4)
