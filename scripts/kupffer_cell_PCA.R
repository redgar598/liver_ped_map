...
## only Healthy KC
d10x.combined_myeloid<-subset(d10x.combined, subset = CellType_refined %in% c("KC Like"))
rm(d10x.combined)
gc()

d10x.combined_myeloid<-subset(d10x.combined_myeloid, subset = age_condition %in% c("Ped Healthy","Adult Healthy"))
...
## KC PCA
#####################
d10x.combined_myeloid

DimPlot(d10x.combined_myeloid, reduction="pca")
...
FeaturePlot(d10x.combined_myeloid, reduction="pca", features = "MARCO")
FeaturePlot(d10x.combined_myeloid, reduction="pca", features = "ALB")
...
FeaturePlot(d10x.combined_myeloid, reduction="pca", features =c("IL1B","CXCL3","CCL4L2","CCL4"), dims=c(5,6))
FeaturePlot(d10x.combined_myeloid, reduction="pca", features =c("CD209","SLC40A1","ETV5","VCAM1"), dims=c(5,6))

DimPlot(d10x.combined_myeloid, reduction="pca", group.by="age_condition",dims=c(5,6))+colscale_agecondition
VlnPlot(d10x.combined_myeloid, features = 'PC_5', group.by="age_condition")
RidgePlot(d10x.combined_myeloid, features = 'PC_5', group.by="age_condition")
...
meta_categorical <- d10x.combined_myeloid@meta.data[, c("Tissue","chemistry","Sex","age_condition","AgeGroup")]  # input column numbers in meta that contain categorical variables
...
expression<-GetAssayData(d10x.combined_myeloid)
...
DimPlot(d10x.combined_myeloid, reduction="pca", group.by="age_condition",dims=c(1,2))+colscale_agecondition

plt_varimax<-data.frame(as.matrix(df$rotScores))
plt_varimax_meta<-cbind(plt_varimax,d10x.combined_myeloid@meta.data)
ggplot(plt_varimax_meta, aes(X5, X6, color=age_condition))+geom_point()+colscale_agecondition
...
summary(aov(plt_varimax_meta$X5~plt_varimax_meta$age_condition))

identical(rownames(embed),rownames(plt_varimax_meta))
cor(embed$PC_5, plt_varimax_meta$X5)

summary(aov(embed$PC_5~plt_varimax_meta$age_condition))
...
### PC5 loadings
gene_list = as.data.frame(unclass(df$rotLoadings))$PC_5
