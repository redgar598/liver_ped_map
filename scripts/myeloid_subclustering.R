...
###########
## DGE Adult specific KC-like cells versus more mixed KC-like cells
###########
...
de<-FindMarkers(d10x_raw_myeloid, ident.1 = "1", ident.2 = "2", test.use = "MAST",latent.vars="nFeature_RNA", verbose=F)
sig_de<-de[which(de$p_val_adj < 0.005 & abs(de$avg_log2FC) > 1),]

FeaturePlot(d10x.combined_myeloid, features = c("ATP5E","CD163","MTRNR2L12","TTR"), min.cutoff = "q9", pt.size=1)


sig_up_in_adult<-sig_de[which(sig_de$avg_log2FC<0),]
FeaturePlot(d10x.combined_myeloid, features = c("MTRNR2L12","S100A9","IFI27","ALB"), min.cutoff = "q9", pt.size=1)
...
FeaturePlot(d10x.combined_myeloid, features = c("CCL4","FOS","MTRNR2L12","ALDOB"), min.cutoff = "q9", pt.size=0.1)
de[grep("CCL|FOS|ALDOB",row.names(de)),]
...
save_plts(GO_KC_vs_recentrcruti, "GO_KC_adultspeific_versus_allKC", w=20,h=10)

DimPlot(d10x.combined_myeloid, group.by = "Phase", pt.size = 2)
table(d10x.combined_myeloid$seurat_clusters, d10x.combined_myeloid$Phase)
DimPlot(d10x.combined_myeloid, group.by = "cell_status", pt.size = 2)
table(d10x.combined_myeloid$seurat_clusters, d10x.combined_myeloid$cell_status)
FeaturePlot(d10x.combined_myeloid, features = c("nFeature_RNA","percent.mt","nuclear_fraction","nCount_RNA"), pt.size=0.1)
