...
save_plts(roughcell_cluster_tsne, "rPCA_roughcellType_cluster_tsne_nolab", w=6,h=4)
...
save_plts(nuclearfraction_cluster_umap, "rPCA_nuclearfraction_cluster_umap", w=6,h=4)
...
save_plts(damaged_empty_cluster_umap, "rPCA_damaged_empty_cluster_umap", w=6,h=4)
...
save_plts(nFeature_cluster_umap, "rPCA_nFeature_cluster_umap", w=6,h=4)
...
# ## donor integrated map, clusters needing checks: 11, 18, 25,26
# 11 and 25 high Mt not sure about 18 and 26
mt_percent_box<-ggplot(d10x.combined@meta.data, aes(seurat_clusters, percent.mt))+geom_violin( fill='lightgrey')+theme_bw()
save_plts(mt_percent_box, "mt_percent_box", w=12,h=2)
...
save_plts(hep_QC, "ALB_rPCA_UMAP", w=6,h=5)



# find all markers of cluster 18
cluster18.markers <-  FindMarkers(d10x.combined, ident.1 = 18, min.pct = 0.25)
head(cluster18.markers, n = 10)
# seems to be red blood cells
RBC<-FeaturePlot(d10x.combined, features = c("HBB","HBA2","HBA1","FCGR3A"), min.cutoff = "q9", pt.size=0.15)
save_plts(RBC, "RBC_rPCA_UMAP", w=8,h=7)
...
d10x.combined@meta.data$CellType_rough[which(d10x.combined@meta.data$seurat_clusters=="25")]<-"Low Quality"
d10x.combined@meta.data$CellType_rough[which(d10x.combined@meta.data$seurat_clusters=="18")]<-"Erythrocytes"
...
save_plts(individual_split, "individual_roughCell_facet_rPCA_UMAP", w=22,h=14)
...
## RBC
##############
d10x.combined_RBC<-subset(d10x.combined, subset = seurat_clusters %in% c(18))
...
save_plts(RBC_overlap, "RBC_receptor_overlap_rPCA_UMAP", w=14,h=4)
...
save_plts(MAST_markers, "MAST_markers_rPCA_UMAP", w=8,h=4)

MAST_markers<-FeaturePlot(d10x.combined, features = c("TPSAB1", "AREG"))
save_plts(MAST_markers, "MAST_markers_rPCA_UMAP_fullmap", w=8,h=4)


## non-RBC  in 18
DimPlot(d10x.combined_RBC, reduction = "umap", pt.size=1, label=T)
cluster2.markers <-  FindMarkers(d10x.combined_RBC, ident.1 = 2, min.pct = 0.25)
head(cluster2.markers, n = 20)
FeaturePlot(d10x.combined_RBC, features = c("TRBC2", "CD3E"))

DotPlot(object = d10x.combined_RBC, features = T_genes)+xlab("T Cell Marker")
DotPlot(object = d10x.combined_RBC, features = Macrophage_genes)+xlab("Macrophage Marker")
FeaturePlot(d10x.combined_RBC, features = c("LYZ", "S100A8","CD3D","IL32"))


DimPlot(d10x.combined_RBC, reduction = "umap",group.by="cell_status", pt.size=1)
FeaturePlot(d10x.combined_RBC, reduction = "umap",features="percent.mt", pt.size=1, label=T)
FeaturePlot(d10x.combined_RBC, reduction = "umap",features="nFeature_RNA", pt.size=1, label=T)
...
d10x.combined_RBC@meta.data$CellType_rough[which(d10x.combined_RBC@meta.data$seurat_clusters%in%c("0","1"))]<-"Erythrocytes"
...
d10x.combined_RBC@meta.data$CellType_rough[which(d10x.combined_RBC@meta.data$seurat_clusters%in%c("2"))]<-"Doublet"
...
save_plts(myeloid_cluster_umap_nf, "myeloid_cluster_umap_nuclearfraction", w=5,h=4)
...
save_plts(myeloid_cluster_umap_cellstatus, "myeloid_cluster_umap_cellstatus", w=6,h=4)
...
FeaturePlot(d10x.combined_myeloid, features = LEC_genes, min.cutoff = "q9", pt.size=0.25)
...
save_plts(marker_mye, "myeloid_dotplot_doublets", w=20,h=5)
DefaultAssay(d10x.combined_myeloid) <- "integrated"
...
save_plts(bcell_cluster_umap_cellstatus, "bcell_cluster_umap_cellstatus", w=6,h=4)
umap_MTbcell<-FeaturePlot(d10x.combined_bcell, features = "percent.mt", min.cutoff = "q9", pt.size=1)
save_plts(umap_MTbcell, "umap_MTbcell", w=5,h=4)
...
save_plts(tcell_cluster_umap_cellstatus, "tcell_cluster_umap_cellstatus", w=6,h=4)
umap_MTtcell<-FeaturePlot(d10x.combined_NK_T, features = "percent.mt", min.cutoff = "q9", pt.size=1)
save_plts(umap_MTtcell, "umap_MTtcell", w=5,h=4)
...
save_plts(phase_tcell, "umap_phase_tcell", w=5,h=4)

phase_marker<-FeaturePlot(d10x.combined_NK_T, reduction = "umap", features = c("MKI67","TOP2A"), ncol = 2)
save_plts(phase_marker, "umap_phaseMarker_tcell", w=10,h=4)
...
d10x.combined_NK_T@meta.data$CellType_rough[which(d10x.combined_NK_T@meta.data$seurat_clusters%in%c("1","2"))]<-"CD3+ T-cells"
d10x.combined_NK_T@meta.data$CellType_rough[which(d10x.combined_NK_T@meta.data$seurat_clusters%in%c("3"))]<-"gd T-cells"
d10x.combined_NK_T@meta.data$CellType_rough[which(d10x.combined_NK_T@meta.data$seurat_clusters%in%c("5","6"))]<-"Doublet"
...
NKT_cluster_umap
save_plts(NKT_cluster_umap, "NKT_cluster_umap_labelled", w=6,h=4)
...
levels(d10x.combined@meta.data$CellType_refined)<-c("CD3+ T-cells","Cholangiocytes", "Cycling T-cells","Doublet","Erythrocytes",
                                                    "gd T-cells","Hepatocytes","HSC","KC Like","Low Quality","LSEC","Macrophage\n(MHCII high)",
                                                    "Mast cell", "Mature B-cells","Myeloid Erythrocytes\n(phagocytosis)", "Neutrophil",
                                                    "NK-like cells", "Plasma cells","RR Myeloid")
...
d10x.combined@meta.data$CellType_refined<-factor(d10x.combined@meta.data$CellType_refined, levels = c("Mature B-cells","Plasma cells",
                                                                                                      ...
                                                                                                      "Cholangiocytes","LSEC",
                                                                                                      "RR Myeloid","Macrophage\n(MHCII high)","KC Like","Neutrophil","Mast cell",
                                                                                                      "Myeloid Erythrocytes\n(phagocytosis)","Erythrocytes",
                                                                                                      "HSC","Hepatocytes","Doublet","Low Quality"))

all_refined_cluster_umap<-DimPlot(d10x.combined, reduction = "umap", pt.size=0.25, label=T,label.size = 2, group.by = "CellType_refined")+colscale_cellType+ggtitle("")+xlab("UMAP 1")+ylab("UMAP 2")+
...
##############
save(d10x.combined, file=paste(here("data/"),"adult_ped_integrated_refinedlabels_withDropletQC.rds", sep=""))
cell_label<-d10x.combined@meta.data
save(cell_label, file=paste(here("data/"),"adult_ped_cellRefined_withDropletQC.rds", sep=""))
