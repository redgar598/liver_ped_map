...
cell_label$index<-rownames(cell_label)
cell_label<-cell_label[match(colnames(d10x.combined), cell_label$index),]
identical(colnames(d10x.combined), cell_label$index)

d10x.combined <- AddMetaData(d10x.combined, metadata = cell_label)

DimPlot(d10x.combined, reduction = "umap",group.by="CellType_rough", pt.size=0.25, label=T)